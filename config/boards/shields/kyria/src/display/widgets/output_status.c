...
static void default_status_symbol(lv_obj_t *label, struct output_status_state state) {
	char text[10] = {};
...
static void fancy_status_symbol(lv_obj_t *icon, struct output_status_state state) {
	switch (state.selected_endpoint) {
	case ZMK_ENDPOINT_USB:
...
static void set_status_symbol(lv_obj_t *obj, struct output_status_state state) {
	default_status_symbol(obj, state);
}
